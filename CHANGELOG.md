...
## Reroute Enabler UI (mode toggle + Reroute form in ReplanInput)
...
- **Files changed:** `frontend/src/components/ReplanInput.jsx` (rewritten), `frontend/src/App.jsx` (new `handleWorkspaceReroute`, updated `EntryDetailView` signature and call-site), `CHANGELOG.md` (heading rename + this entry), `tests/test_anomaly_replan.py` renamed to `tests/test_reroute_enabler.py` with module docstring updated.
