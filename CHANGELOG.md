...
## Reroute Enabler (scoped-down: exclude_norad_ids + start-position override)
...
## Reroute Enabler UI (mode toggle + Reroute form, fuel slider, tab/counter sync)

**Built with IBM Bob.**

- **Context:** Frontend for Reroute Enabler (originally shipped as the scoped-down "Anomaly Replan" — see above: full mission-state-tracker version was cut for schedule risk, shipped instead as `exclude_norad_ids` + start-position override on the existing `/replan` path, no new endpoint, no session persistence). Two passes merged into this entry: the initial mode-toggle/form build, then a follow-up adding a fuel-remaining control and fixing tab/history bookkeeping around Replan/Reroute/Fix actions.
- **Renamed:** Feature previously called "Anomaly Replan" is now "Reroute Enabler" throughout labels, comments, docs, and the CHANGELOG heading. The underlying backend fields (`start_altitude_km`, `start_inclination_deg`, `exclude_norad_ids`) and the `/replan` endpoint are unchanged — only the human-facing name changed.
- **Frontend (ReplanInput.jsx):** Added a two-button mode toggle at the top of the replan card — "Replan" (left) and "Reroute" (right). Replan mode is unchanged: free-text textarea, submit calls `/replan` with `user_request_text`.
- **Frontend (Reroute mode):** Altitude (`200–2000 km`, step 0.1) and inclination (`0–180 deg`, step 0.1) number inputs, each with a range slider synced in both directions. On first mode-open, both inputs are prefilled from the last non-depot object in the active plan's `route_details`, resolved against `debrisField` for real orbit values. The depot entry (norad_id -1) is excluded from prefill and all Reroute UI elements. Altitude step was originally `1`, changed to `0.1` on both the number and range inputs — native HTML5 validation was rejecting decimal km values on submit.
- **Frontend (fuel-remaining field):** `fuelKm` state wired into the same two prefill points as altitude/inclination — the Match-button snap effect and the globe-click effect (not wired into manifest leg clicks, left untouched on purpose). Prefill = `baseEntry.params.fuel_budget_km_s` (ceiling) minus the cumulative `delta_v_km_s` across route legs up to the clicked object, derived live at click-time — no separate snapshot/ceiling field stored on the history entry, avoiding any conflict with the existing `overrides_applied` fold-back logic. Clamped to `[0, ceiling]`. Ceiling shown as an inline soft-cap label (visual only); hard input bounds `min=0 max=15`. `handleWorkspaceReroute` sends the value as `fuel_budget_km_s` inside `applied_proposal` — zero backend change, field already accepted.
- **Frontend (snap-to-object):** One "Match [name]" button is generated for each non-depot object in `route_details`. Clicking copies that object's real `altitude_km`/`inclination_deg` (looked up from `debrisField` by `norad_id`) into the inputs, plus the fuel-remaining prefill above. Button is disabled when orbit data is unavailable; tooltip shows the exact values when available.
- **Frontend (globe-click position input):** Clicking a tracked object on the CesiumJS globe while Reroute mode is open copies its `altitude_km`/`inclination_deg` (and fuel-remaining, per above) into the inputs via the existing `handleDebrisSelect` → `activeDebrisId` path — no new Cesium event handler added. Clicking empty space produces `null`, which is correctly ignored. Only real tracked objects carry valid orbit data; untracked positions (trajectory legs, interpolated positions) are not clickable and are not supported.
- **Frontend (exclude checklist):** Checkboxes generated from non-depot `route_details` entries. Checking an object adds its `norad_id` to `excludedIds`. No free-text ID entry — the list is entirely derived from the current route.
- **Frontend (submit):** "Apply changes" button is shared between modes. Reroute submit calls `handleWorkspaceReroute` which sends `applied_proposal: { start_altitude_km, start_inclination_deg, fuel_budget_km_s }` and `exclude_norad_ids` as a top-level `ReplanRequest` field — bypasses the LLM parser entirely. `_ALLOWED_OVERRIDE_KEYS` in `main.py` is untouched (confirmed by inspection).
- **Frontend (App.jsx — per-kind counters + tab/history sync):** `appendReplanTab` takes a `kind` param (`'replan' | 'reroute' | 'fix'`) instead of a hardcoded label. Three independent ref-based counters (not derived from the last visible tab's label, which would collide once `MAX_ROUTE_REPLAN_TABS` trims old tabs) — `Replan #N` / `Reroute #N` / `Fix #N`. Tab strip and History panel selection are now the same state, synced in both directions: clicking a tab moves `activeWorkspaceId` to the matching history entry, and clicking a history entry moves the active tab to match. Viewing a non-latest entry hides the Replan/Reroute/Fix action panel (read-only) and renders that entry's own snapshot via the existing `EntryDetailView` path — no second rendering path built.
- **Styling (global.css):** `accent-color: var(--c-signal)` scoped to `.replan input[type="number"], .replan input[type="range"]` — covers the fuel slider plus the altitude/inclination pairs, without touching checkboxes or any other component.
- **Known limitation:** Mid-transfer and mid-leg spacecraft positions are not supported as a position source. The only valid position inputs are snap-to-object (which uses a real tracked object's TLE-derived orbit), globe-click on a tracked object (same), or manual numeric entry. Interpolated or estimated positions between route legs are not tracked objects and carry no verified orbit data — consistent with the project's "never invent data" principle.
- **Audit finding:** A follow-up full audit pass (all 4 parts checked against spec) found **zero bugs in the shipped source** (`ReplanInput.jsx`, `App.jsx`, `global.css` all matched spec exactly). All defects found were in the test file itself, not the app: bad `getByDisplayValue` selector matching two inputs with the same value, unflushed `act()` around a mode-switch effect, `fireEvent.click` on a `type="submit"` button not reliably firing `onSubmit` in jsdom, a submit-button test failing because App auto-switches away from the Parameters panel after the first plan generation, and an imprecise `queryByText('Replan')` check for "is the action panel hidden" (fixed to check for the `.workspace-replan` wrapper directly).
- **Testing:** `tests/reroute-panel.test.jsx` rewritten, 8 tests: 6 covering fuel prefill/ceiling math (ceiling label, Match-button prefill on two different objects, globe-click prefill, clamp-to-0, submit sends clamped value), 1 covering old-tab read-only snapshot behavior, 1 covering per-kind counters surviving a tab trim. All 8 passing, individually confirmed by name via `npx vitest run tests/reroute-panel.test.jsx --reporter=verbose` after an initial default-reporter run truncated the visible test list (a vitest default-reporter display quirk affecting every suite in this project, not specific to this file — noted for future verification passes: use `--reporter=verbose` when a full per-test confirmation is needed).
- **Files changed:** `frontend/src/components/ReplanInput.jsx`, `frontend/src/App.jsx` (`handleWorkspaceReroute`, `EntryDetailView` signature and call-site, per-kind counters), `frontend/src/styles/global.css`, `CHANGELOG.md` (heading rename + this entry), `tests/test_anomaly_replan.py` renamed to `tests/test_reroute_enabler.py` with module docstring updated.