...
Orbits are treated as circular throughout `delta_v.py`, not real elliptical/osculating elements. Adding real eccentricity was evaluated and deliberately not built: vis-viva speed at a point on an orbit depends on the object's actual radius `r` at that point (i.e. where it currently sits on its ellipse), which requires modeling orbital position/timing (true anomaly) — explicitly out of scope (see the RAAN-drift-vs-wait-time bullet below for the same scoping boundary applied elsewhere). Without real position data, the only value available for `r` is the semi-major axis `a` itself, and vis-viva algebraically reduces to the existing circular-velocity formula whenever `r = a` — so adding eccentricity without also adding position modeling would change zero delta-v numbers while adding real complexity and a false impression of improved accuracy. Kept as circular, honestly, rather than shipping a change that does nothing.
